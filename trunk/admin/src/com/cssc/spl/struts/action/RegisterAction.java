...
public class RegisterAction extends Action {
    ...
    private final String PAYMENT_OPTION_DATA = "PAYMENT_OPTION_DATA";
        ...
        try {
            ...
            if (SPEC_REGISTER_SUBMIT_MAP.equals(path)) {
                ...
                if (errors.isEmpty()) {
                    ...
                    errorIter = errors.get (ERRORS+".label.paymentdate");
                    while (errorIter.hasNext()) {
                        ActionMessage message = (ActionMessage) errorIter.next();
                        messages.add (ERRORS + ".label.paymentdate", message);
                    }
                    errorIter = errors.get (ERRORS+".label.bankname");
                    while (errorIter.hasNext()) {
                        ActionMessage message = (ActionMessage) errorIter.next();
                        messages.add (ERRORS + ".label.bankname", message);
                    }
                    errorIter = errors.get (ERRORS+".label.checknbr");
                    while (errorIter.hasNext()) {
                        ActionMessage message = (ActionMessage) errorIter.next();
                        messages.add (ERRORS + ".label.checknbr", message);
                    }
    ...
    private ActionMessages handleListSpecRegister (RegisterForm registerForm, HttpServletRequest request) throws CSSCApplicationException, CSSCSystemException {
        ...
        String paymentOptionData = Constants.getProperty (PAYMENT_OPTION_DATA);
        LabelValueBean[] paymentOptionBeans = CSSCUtil.getLabelValueBeans(paymentOptionData, ":", ";");
        registerForm.setPaymentoptionBeans (paymentOptionBeans);
    ...
    private ActionMessages handleSpecRegister (RegisterForm registerForm, HttpServletRequest request) throws CSSCApplicationException, CSSCSystemException {
        ...
        ActionMessages messages = new ActionMessages ();
        UserVO userVO = registerForm.getUserVO();
        String paymentoption = registerForm.getPaymentoption();
        if (!DEF.equals(paymentoption) && !DNC.equals (paymentoption)) {
        ...
        if (DEF.equals(paymentoption)) {
            userVO.setStatus(NEW);
        }
        
        if (DNC.equals(paymentoption)) {
            userVO.setStatus(ACTV);
        }
    ...
    private ActionMessages handlePay (RegisterForm registerForm, HttpServletRequest request, UserVO userVO) throws CSSCApplicationException, CSSCSystemException {
        ...
        if (CASH.equals(paymentoption)) {
            messages = handleCashPayment (registerForm, userVO);    
        }
        
        if (CHECK.equals(paymentoption)) {
            messages = handleCheckPayment (registerForm, userVO);    
        }
        
        if (CC.equals(paymentoption)) {
            messages = handleCCPayment (registerForm, userVO);
        }
    ...
    private ActionMessages handleCashPayment (RegisterForm registerForm, UserVO userVO) throws CSSCApplicationException, CSSCSystemException {
        ActionMessages messages = new ActionMessages ();
        long invoiceNbr = Calendar.getInstance().getTimeInMillis();        
        double amount = registerForm.getSetupFee();
        String paymentDate = registerForm.getPaymentdate();        
        Date paymentdate = CSSCUtil.parseDate(paymentDate, "MM/dd/yyyy");
        
        PaymentVO paymentVO = new PaymentVO ();
        paymentVO.setTransactionId(invoiceNbr);
        ...
        paymentVO.setResponseCode(1);
        paymentVO.setApprovalCode(0);
        paymentVO.setResponseTxt("SUCCESS");
        paymentVO.setAvsCode("0");
    ...
    private ActionMessages handleCheckPayment (RegisterForm registerForm, UserVO userVO) throws CSSCApplicationException, CSSCSystemException {
        ActionMessages messages = new ActionMessages ();
        long invoiceNbr = Calendar.getInstance().getTimeInMillis();       
        double amount = registerForm.getSetupFee();
        String paymentDate = registerForm.getPaymentdate();        
        Date paymentdate = CSSCUtil.parseDate(paymentDate, "MM/dd/yyyy");
        String refNbr = registerForm.getChecknbr();
        String avsCode = registerForm.getBankname();
        
        PaymentVO paymentVO = new PaymentVO ();
        paymentVO.setTransactionId(invoiceNbr);
        paymentVO.setPaymentType(CHECK);
        paymentVO.setRefNbr(refNbr);
        paymentVO.setInvoiceNbr(String.valueOf(invoiceNbr));
        paymentVO.setLocationName("SETUP");
        paymentVO.setUserId(userVO.getUsername());
        paymentVO.setAmount(amount);
     ...
     * @throws com.cssc.spl.exception.CSSCApplicationException
     */
    private ActionMessages handleCCPayment (RegisterForm registerForm, UserVO userVO) throws CSSCSystemException, CSSCApplicationException {
        ActionMessages messages = new ActionMessages ();
        long invoiceNbr = Calendar.getInstance().getTimeInMillis();        
        double amount = registerForm.getSetupFee();
        ...
        Date paymentDate = new Date (invoiceNbr);
        paymentVO.setPaymentDate(paymentDate);
        paymentVO.setUserId(userVO.getUsername());
        paymentVO.setCcType(registerForm.getCctype());
        paymentVO.setExpMon(Integer.parseInt (expMon));
        paymentVO.setExpYear (Integer.parseInt (expYear));
        paymentVO.setSecCode(Integer.parseInt (secCode));
        paymentVO.setPaymentType(CC);
